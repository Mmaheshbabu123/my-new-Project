...
const ListView = (props) => {
	...
	return (
		<li key={props.index} className= {`list-inline-item ${props.level=='3'?styles.section_func_li:styles.sectionli} ms-2 my-2 hi sectioncolor`}>
			{header && (
				<div
				...
				>
					<div className={`accordion-item `}>
						...
						id={`flush-heading${props.pc_number.replace(".", "")}pc${props.index}`}>
							<button
								className={`accordion-button  poppins-regular-16px color-gray-lt collapsed ${styles.sectioncolor} `}
								type="button"
							...
							>
								{props.sectype=='pc' &&<span className="w-25 poppins-regular-16px"> {props.pc_number}</span>}
								<span className={props.sectype=='pc'?'ms-4 poppins-regular-16px':''}>{props.title}</span>
							</button>
						</h4>
						<div
