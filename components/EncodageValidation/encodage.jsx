...
function EncodageValidation(props) {
	...
	const filter = () => {
		APICALL.service(
			process.env.NEXT_PUBLIC_APP_BACKEND_URL + '/api/filter-encodage-data',
			'POST',
	...
	const updateValidation = (wid,start,stop) => {
				...
				rows.push(
					<tr className="border poppins-regular-18px p-2">
						...
						<td style={{ width: '149px' }} className="align-middle p-2 poppins-light-16px">
							<button
								onClick={() => updateValidation(element.wid, element.starttime, element.endtime)}
